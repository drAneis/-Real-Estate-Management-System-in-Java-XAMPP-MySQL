...
public class ManageProduct extends JFrame implements ActionListener {
	...
	private JLabel title, header, productIdLabel, productNameLabel, productQtLabel, productPriceLabel, userIdLabel, productPhoneLabel, productEmailLabel ;
	private JTextField productIdTF, productNameTF, productQtTF, productPriceTF, userIdTF, productPhoneTF, productEmailTF;
	...
	public ManageProduct(String pid, ViewProductActivity prev) {
		...
		productPhoneLabel = new JLabel("Phone: ");
		productPhoneLabel.setBounds(60, 140, 140, 30);
		productPhoneLabel.setVisible(true);
		productPhoneLabel.setFont(Theme.FONT_INPUT);
		panel.add(productPhoneLabel);
		

		productEmailLabel = new JLabel("Email: ");
		productEmailLabel.setBounds(60, 180, 140, 30);
		productEmailLabel.setVisible(false);
		productEmailLabel.setFont(Theme.FONT_INPUT);
		...
		productPhoneTF = new JTextField(product.getPhone()+"");
		productPhoneTF.setBounds(180, 140, 220, 30);
		productPhoneTF.setFont(Theme.FONT_INPUT);
		productPriceTF.setVisible(false);
		panel.add(productPhoneTF);
		
		productEmailTF = new JTextField(product.getEmail()+"");
		productEmailTF .setBounds(180, 180, 220, 30);
		productEmailTF .setFont(Theme.FONT_INPUT);
		productEmailTF .setVisible(false);
		panel.add(productEmailTF);
		
		
		
		buttonEdit = new JButton("Edit");
		buttonEdit.setBounds(180, 260, Theme.BUTTON_PRIMARY_WIDTH,30);
		...
		buttonSubmit.setBounds(180, 260, Theme.BUTTON_PRIMARY_WIDTH,30);
		...
		buttonDelete.setBounds(300, 260, Theme.BUTTON_PRIMARY_WIDTH,30);
	...
	public void actionPerformed(ActionEvent ae) {
		if (ae.getSource().equals(buttonSell)) {
			try {
				product.sellProduct(userIdTF.getText().trim());
			...
			productPhoneTF.setEnabled(true);
			productEmailTF.setEnabled(true);
			userIdLabel.setVisible(false);
			userIdTF.setVisible(false);
			productPriceLabel.setVisible(true);
			productPriceTF.setVisible(true);
			productPhoneLabel.setVisible(true);
			productPhoneTF.setVisible(true);
			productEmailLabel.setVisible(true);
			productEmailTF.setVisible(true);
		}
		else if (ae.getSource().equals(buttonSubmit)) {
			try {
				product.updateProduct(productNameTF.getText(),Double.parseDouble(productPriceTF.getText()),Integer.parseInt(productPhoneTF.getText()), productEmailTF.getText() );
				if (!prev.keywordTF.getText().trim().isEmpty())
					prev.table.setModel(Product.searchProduct(prev.keywordTF.getText().trim(), prev.byWhatCB.getSelectedItem().toString()));
				else
					prev.table.setModel(Product.searchProduct("", "By Name"));
