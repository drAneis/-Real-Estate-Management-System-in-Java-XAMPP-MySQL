...
public class ViewProductActivity extends JFrame implements ActionListener {
	...
	public ViewProductActivity(JFrame prev, Employee employee) {
		...
		buttonAddProduct = new JButton("添加");
		...
		table.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent evt) {
				...
				System.out.println("getClickCount() = " + evt.getClickCount()); //debuging statement
				if (evt.getClickCount() == 2) {
					//class com o codigo da fotografia
					
					
					  System.out.println("Completed : doubleClick()");//debuging statement
					  System.out.println("getClickCount() = " + evt.getClickCount());//debuging statement
					  jTable_Clickedf(evt);
				}
					   ...
					   jTable_ClickMouseClicked(evt);
					   System.out.println("Completed : tripleClick()"); //debuging statement
				}
	...
	private void jTable_Clickedf(MouseEvent evt) {
	    int index = table.getSelectedRow();
	    
	   // TableModel model = table.getModel();

	   // int value1 = getValueAt(index, 0);
	    
		ImageF gui = new ImageF(index);
		//gui.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		gui.setVisible(true);
		gui.pack();
		gui.setTitle("House Images");
